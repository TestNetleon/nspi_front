import axios from 'axios';
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import '../App.css';
import Navbar from './Navbar';
import Sidebar from './Sidebar';
var cors = require('cors')


const HelpTopic = () => {

 const [helptopic, setTopics] = useState([]);
 useEffect(() => {
    getTopics();
  }, [])


  const getTopics = async () => {
    axios.get(process.env.REACT_APP_BASE_URL + "/helptopics", {
    })
      .then(function (response) {
        setTopics(response.data);
        console.log(response);
      })
      .catch(function (error) {
        // setError(error);
        console.log(error);
      });
  };

  //delete data
  const deleteTopic = async (id) => {
    console.log(id);
    let a = window.confirm('Are you sure you want to delete it..');
    if (a) {
      axios.delete(process.env.REACT_APP_BASE_URL + `/delete_topic/${id}`)
      .then(function (response) {
        console.log(response);
        getTopics();
      })
      .catch(function (error) {
        // setError(error);
        console.log(error);
      });
     
    }
    else {

    }
  }

  


    return (
        <div className="h-100">
          <div className="vertical-layout h-100 vertical-menu-modern menu-expanded navbar-floating footer-static">
            <Navbar />
            <Sidebar />
            <div className="app-content content">
              <div className="content-overlay" />
              <div className="header-navbar-shadow" />
              <div className="content-wrapper">
                <div className="content-body">
                  <section id="dashboard-ecommerce">
                    {/* content starts */}
                    <h2>Help Topics</h2>
                    <Link to ="/add_helptopic">
                    <button type="button" class="btn btn-primary">Add Topic</button>
                    </Link>
                  
                    
                       <table class="table">
                        <thead>
                          <tr>
                            <th>S no.</th>
                            <th>Topic Name</th>
                            <th>Action</th>
                        
                           
                            </tr>
                          </thead>
                          <tbody>
                            {
                            helptopic.map((item, index) =>       
                            <tr key={item._id}>
                            <td>{index + 1}</td>
                            <td>{item.name}</td>
                            <td>
                            <Link to ={"/update_helptopic/"+item._id}><button type="button" class="btn btn-primary">Edit</button></Link>      
                             <button type="button" class="btn btn-primary" onClick={() => deleteTopic(item._id)}>Delete</button>
                             </td>
                          
                            </tr>
                            )
                            }
                        
                        </tbody>
                      </table>
                    {/* content ends */}
                  </section>
                </div>
              </div>
            </div>
          </div>
        </div>
      )
}

export default HelpTopic